...
namespace TrumpSoftware.Common.Log
{
    public static class DebugHelper
    {
        ...
        {
            Debug.WriteLine("({0}) Task='{1}'. Enter to {2}.{3}", DateTime.Now.ToString("HH:mm:ss.ffff"), Task.CurrentId, classType.Name, memberName);
        }

        public static void LeaveFromMember(Type classType, [CallerMemberName] string memberName = null)
        {
            Debug.WriteLine("({0}) Task='{1}'. Leave from {2}.{3}", DateTime.Now.ToString("HH:mm:ss.ffff"), Task.CurrentId, classType.Name, memberName);
        }
        ...
        {
            Debug.WriteLine("({0}) Task='{1}'. In method '{2}.{3}' variable {4}='{5}'", DateTime.Now.ToString("HH:mm:ss.ffff"), Task.CurrentId, classType.Name, memberName, variableName, value);
        }
